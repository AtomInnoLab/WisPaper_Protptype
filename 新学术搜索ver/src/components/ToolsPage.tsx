...
export function ToolsPage({ onOpenTrueCite }: ToolsPageProps) {
  return (
    <main className="min-h-screen flex-1 overflow-y-auto bg-[#f7f9fc] px-8 py-12">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-9 grid gap-5 md:grid-cols-2">
          ...
          <article className="relative overflow-hidden rounded-3xl border border-dashed border-slate-300 bg-white/70 p-7">
            <div className="flex items-start justify-between gap-5">
              <div className="grid h-12 w-12 place-items-center rounded-2xl bg-violet-50 text-violet-600"><FileImage className="h-6 w-6" /></div>
              <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-500">即将上线</span>
            </div>
            ...
            <button disabled className="mt-5 rounded-xl bg-slate-100 px-4 py-2 text-xs font-semibold text-slate-400">敬请期待</button>
          </article>
        </div>
      </div>
    </main>
